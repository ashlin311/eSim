...
function installNghdl
{
    ...
    #TO avoid re-extracting NGHDL to prevent overwriting local modifications
    if [ ! -d "nghdl" ]; then
    echo "Extracting NGHDL..."
    unzip nghdl.zip
    else
    echo "NGHDL already extracted. Skipping unzip."
    fi
    cd nghdl/
...
function installKicad
{
    ...
    sudo rm -f /etc/apt/sources.list.d/*kicad* 2>/dev/null
    #Used system KiCad package instead of PPA
    sudo apt-get update || {
...
function copyKicadLibrary
{
    ...
    #Used kicad-cli for version detection (kicad --version unsupported) and dynamically set config path
    kicad_version=$(kicad-cli version | cut -d'.' -f1)
    mkdir -p ~/.config/kicad/$kicad_version.0

    # Copy symbol table for eSim custom symbols 
    cp kicadLibrary/template/sym-lib-table ~/.config/kicad/$kicad_version.0
